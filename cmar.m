function rules = cmar(train_data, train_labels, min_support, min_confidence)
    ...
    % Generate multiple-antecedent rules from frequent itemsets
...
function rules = generate_rules(freq_itemsets, min_confidence, data_table)
    ...
    for i = 1:length(freq_itemsets)
        ...
        for j = 1:length(itemset)-1
            antecedents = combnk(itemset, j);
            for k = 1:size(antecedents, 1)
